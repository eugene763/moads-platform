...
### 2.3 Dodo launch phase
- current live billing mode: `Free + Packs only`,
- `Pack S / M / L` are one-time Dodo purchases,
...
Billing provider:
- Dodo Payments is the active provider for AEO credit packs in this phase.
- Canonical webhook endpoint: `POST /v1/billing/webhooks/dodo`

## 6) Security and data rules

- secrets only server-side (Secret Manager / env, not frontend),
- Dodo credentials live only in:
  - `DODO_API_KEY`
  - `DODO_WEBHOOK_KEY`
...
8. Dodo packs can be wired without enabling Starter subscriptions.
