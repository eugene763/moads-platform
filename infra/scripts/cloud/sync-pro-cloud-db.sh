...
"$PROXY_BIN" "$INSTANCE_CONNECTION_NAME" --port "$PROXY_PORT" >/tmp/moads-cloud-sql-proxy-pro.log 2>&1 &
PROXY_PID=$!
...
if ! nc -z 127.0.0.1 "$PROXY_PORT" >/dev/null 2>&1; then
  echo "Cloud SQL Proxy failed to start. Check /tmp/moads-cloud-sql-proxy-pro.log" >&2
  exit 1
fi
